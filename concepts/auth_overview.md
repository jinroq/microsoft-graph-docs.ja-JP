...
Microsoft Graph を呼び出す場合は、アクセス トークンをベララー トークンとして HTML 要求の Authorization ヘッダーに添付します。その例として、サインインしているユーザーのプロファイル情報を返す呼び出しを示します (読みやすくするために、アクセス トークンは切り詰められています)。
...
Microsoft Graph のアクセス許可の完全なリスト、および委任されたアクセス許可とアプリケーションのアクセス許可の相違点については、「[アクセス許可のリファレンス](permissions_reference.md)」を参照してください。
...
>**重要**: 
>
>**実稼働用のアプリケーション開発するときには、使用するエンドポイントを決定する前に、「[v2.0 エンドポイントの使用が適しているかどうかを判断するには](https://docs.microsoft.com/azure/active-directory/develop/active-directory-v2-limitations)」を参照してください。**
...
### <a name="microsoft-graph-connect-samples"></a>Microsoft Graph Connect のサンプル 

Microsoft は、広範なプラットフォーム (Android、Angular.JS、ASP.NET、iOS (Obj-C および Swift)、Node.JS、PHP, Python、Ruby、UWP、Xamarin など) に対応した Microsoft Graph の Connect サンプルを公開しています。これらのサンプルを使用して、Azure AD からトークンを取得するために各種の認証ライブラリを使用するコードを試験できます。現在、ほとんどのサンプルがサード パーティ製の認証ライブラリを使用していますが、ASP.NET と UWP のサンプルは Microsoft のライブラリを使用しています。

- 「[初めてのアプリをビルドする](get-started.md)」のセクションには、Azure AD v2.0 エンドポイントを使用する Connec アプリの作成方法に関する詳細な記事が記載されています。また、各プラットフォームで使用する認証ライブラリについても説明しています。利用可能なサンプルは、プラットフォームと認証エンドポイントごとにリストされています。
- 目的のプラットフォームですぐにサンプルを実行するには、「[Microsoft Graph クイック スタート](https://developer.microsoft.com/graph/Quick-Start)」を参照してください。
- Microsoft Graph に利用可能なすべてのサンプルについては、GitHub の [Microsoft Graph リポジトリ](https://github.com/microsoftgraph)を参照してください。 
