...
2 番目の要求では、前の応答の `nextLink` を使用しています。それには、`skipToken` が含まれています。 パラメーターと `$expand` パラメーターは、トークンの中にエンコードされているため、明示的には存在しないことに注意してください。`$select`
...
変更があった場合は、変更されたグループのコレクションが含められます。 さらに応答には、`nextLink` (取得対象の変更のページが複数ある場合) と `deltaLink` のどちらかも含まれています。 の後もそれまでと同じパターンを実装し、最後の `deltaLink` を将来の呼び出しに備えて保持しておいてください。`nextLinks`
...
- `members@delta` には、メンバーシップに対して加えられた変更内容が含まれています。
  - リスト中の最初のユーザーが、メンバーシップを削除するか、またはユーザー オブジェクト自体を削除することにより、グループから削除されました。 プロパティがそのことを記述しています。`@removed`
  - 2 番目のユーザーはグループに追加されました。

## <a name="paging-through-members-in-a-large-group"></a>大規模グループ内のメンバー間のページング
プロパティは、`$select` クエリ パラメーターが指定されていない場合、または `$expand=members` パラメーターが明示的に指定されている場合、既定でグループ オブジェクトに含められます。`members@delta` メンバー数の多いグループの場合、1 つの応答に全メンバーが収まらないことがあります。このセクションでは、そのような状況を処理するために実装しなければならないパターンについて説明します。
...
2. をたどる際、再び同じグループ オブジェクトが含まれる応答を受け取ることがあります。`nextLink` 同じプロパティ値が返されますが、展開された `members@delta` プロパティの内容は、異なるユーザー リストになっています。
...
- 特定の応答シーケンスを前提にしないようにするのが最善です。 シーケンスのどこにでも同じグループが出現する可能性があると想定し、マージ ロジックの中でそれを処理します。`nextLink`
