...
5. **Microsoft Graph SDK を使用する場合は**、SDK をインストールして参照してください。  
   a.コマンド プロンプトで、次のコマンドを実行して SDK をインストールします。
        
        bower install https://github.com/microsoftgraph/msgraph-sdk-javascript.git
         
   b.**public/index.html** ファイルで、`<!-- App code. -->` セクションの上にある **head** 要素に次のコードを追加します。
...
4. **Microsoft Graph SDK を使用する場合は**、app.js を開き、ファイルの下部に次のコードを追加します。これにより、SDK が初期化されます。
...
Microsoft Graph SDK を使用する場合は読み進んでください。REST を使用する場合は、「[REST API を使用する](#using-the-rest-api)」をご覧ください。
...
6. 変更をすべて保存します。これで、[アプリを実行する](#run-the-app)準備ができました。
