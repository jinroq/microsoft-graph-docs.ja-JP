...
Microsoft Graph は、[ユーザー](../api-reference/v1.0/resources/user.md)や[メッセージ](../api-reference/v1.0/resources/message.md)などの数多くのリソースを通じて、多様なユーザー中心のデータと分析情報にアクセスするための単一の API エンドポイントを提供します。独自のアプリケーション データを使用して Microsoft Graph を_**拡張**_する方法を現在提供しています。Microsoft Graph リソースに対して、外部のデータ ストアを必要とせずに、カスタム プロパティを追加できます。 
