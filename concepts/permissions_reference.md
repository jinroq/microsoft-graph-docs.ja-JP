...
すべてのアクセス許可がMicrosoft アカウントと職場または学校のアカウントの両方で有効なわけではありません。 特定のアクセス許可が Microsoft アカウント、職場または学校のアカウント、またはその両方に有効かどうかを判断するには、各アクセス許可グループの**  Microsoft Account Supported ** 列を確認します。 

### <a name="user-and-group-search-limitations-for-guest-users-in-organizations"></a>組織内のゲストユーザーに対するユーザーとグループの検索制限
...
|   アクセス許可    |  表示文字列   |  説明 | 管理者の同意が必要​ | サポートされている Microsoft アカウント |
|:----------------|:------------------|:-------------|:-----------------------|:--------------|
...
|アクセス許可    |表示文字列   |説明 |管理者の同意が必要​ |
|:-----------------------------|:-----------------------------------------|:-----------------|:-----------------|
|_Calendars.Read_ |すべてのメールボックスにあるカレンダーの読み取り |サインインしているユーザーなしで、すべてのカレンダーのイベントをアプリで読み取れるようにします。 |はい |
...
|アクセス許可    |表示文字列   |説明 |管理者の同意が必要​ |
|:-----------------------------|:-----------------------------------------|:-----------------|:-----------------|
|_Contacts.Read_ |すべてのメールボックスにある連絡先の読み取り |サインインしているユーザーなしで、すべてのメールボックス内のすべての連絡先をアプリで読み取りできるようにします。 |はい |
...
|   アクセス許可    |  表示文字列   |  説明 | 管理者の同意が必要​ |
|:-----------------------------|:-----------------------------------------|:-----------------|:-----------------|
| _Directory.Read.All_ | ディレクトリ データの読み取り | サインインしているユーザーなしで、ユーザー、グループ、アプリなどの組織のディレクトリ内のデータをアプリで読み取りできるようにします。 | はい |
| _Directory.ReadWrite.All_ | ディレクトリ データの読み取りおよび書き込み | アプリで、サインインしているユーザーなしで、ユーザー、グループなどの組織のディレクトリ内のデータを読み取りと書き込みができるようにします。ユーザーまたはグループの削除はできません。 | はい |
...
> - 具体的には、上記の一覧表に示されてないリソースの作成または更新は除外されます。 これには、次が含まれます: アプリケーション、oAauth2Permissiongrant、appRoleAssignment、デバイス、servicePrincipal、組織、ドメイン等
...
| アクセス許可 | 表示文字列 | 説明 | 管理者の同意が必要​ |
| :--------- | :------------- | :---------- | :--------------------- |
| _Files.Read.All_ | すべてのサイト コレクションにおけるファイルの読み取り | サインイン ユーザーなしで、アプリがすべてのサイト コレクション内のすべてのファイルを読み取れるようにします。  | はい |
...
> **注**：個人のアカウントでは、Files.Read と Files.ReadWrite も、サインインしているユーザーと共有しているファイルへのアクセスを許可します。 
...
|   アクセス許可    |  表示文字列   |  説明 | 管理者の同意が必要​ |
|:-----------------------------|:-----------------------------------------|:-----------------|:-----------------|
| _Group.Read.All_ | すべてのグループの読み取り | サインインのユーザーがいない状態で、すべてのグループのメンバーシップをアプリで読み取れるようにします。 > **注:** すべてのグループ API サポートがアプリ専用の許可を使用して、アクセスするわけではありません。  [既知の問題](../concepts/known_issues.md) で、例を参照してください。 | はい |
| _Group.ReadWrite.All_ | すべてのグループの読み取りと書き込み | アプリでグループの作成、読み取り、グループのメンバーシップの更新、およびグループの削除が出来るようにします。 サインインのユーザなしで、これらの操作のすべてをアプリで実行出来ます。 > **注:** すべてのグループ API サポートがアプリ専用の許可を使用して、アクセスするわけではありません。  [既知の問題](../concepts/known_issues.md) で、例を参照してください。| はい |
...
* _Group.ReadWrite.All_: グループのメンバーを更新 (`POST /groups/{id}/members/$ref`)します。 
> **注:**: _User.ReadBasic.All_ で、メンバーとして加えるユーザーを読み取ることも必要です。
...
|   アクセス許可    |  表示文字列   |  説明 | 管理者の同意が必要​ |
|:-----------------------------|:-----------------------------------------|:-----------------|:-----------------|
| _IdentityRiskEvent.Read.All_ |   ID リスク イベント情報の読み取り | サインインしているユーザーなしで、組織内のすべてのユーザーの ID リスク イベント情報をアプリで読み取れるようにします。 | はい |
...
| _Mail.ReadWrite_ |    すべてのメールボックスにあるメールの読み取りと書き込み | サインインしているユーザーなしで、アプリですべてのメールボックス内のメールの作成、読み取り、更新、削除を行えるようにします。メールを送信するためのアクセス許可は含まれません。 | はい |
| _Mail.Send_ |    任意のユーザーからのメールを送信 | サインインしているユーザーなしで、任意のユーザーとしてアプリでメールを送信できるようにします。 | はい | 
...
|   アクセス許可    |  表示文字列   |  説明 | 管理者の同意が必要​ |
|:----------------|:------------------|:-------------|:-----------------------|
| _Member.Read.Hidden_ | すべての非表示のメンバーシップの読み取り | サインインしているユーザーなしで、非表示のグループのメンバーシップと管理単位をアプリで読み取れるようにします。 | はい |
...
|   アクセス許可    |  表示文字列   |  説明 | 管理者の同意が必要​ |
|:----------------|:------------------|:-------------|:-----------------------|
| _Notes.Read.All_ |    すべての OneNote ノートブックの読み取り | サインインしているユーザーなしで、組織内のすべての OneNote ノートブックをアプリで読み取れるようにします。 | はい |
| _Notes.ReadWrite.All_ |    すべての OneNote ノートブックの読み取りと書き込み | サインインしているユーザーなしで、組織内のすべての OneNote ノートブックをアプリで読み取り、共有、および変更できるようにします。| はい |
...
|   アクセス許可    |  表示文字列   |  説明 | 管理者の同意が必要​ | サポートされている Microsoft アカウント |
|:----------------|:------------------|:-------------|:-----------------------|:--------------|
| _電子メールを送信する_ |    ユーザーのメール アドレスの表示 | ユーザーのプライマリ電子メール アドレスをアプリで読み取れるようにします。 | いいえ | いいえ |
...
| _プロフィール_ |    ユーザーの基本プロファイルの表示 | ユーザーの基本プロファイル (名前、写真、ユーザー名) をアプリで確認できるようにします。| いいえ | いいえ |
...
|   アクセス許可    |  表示文字列   |  説明 | 管理者の同意が必要​ |
|:-----------------------------|:-----------------------------------------|:-----------------|:-----------------|
| _People.Read.All_ | すべてのユーザーに関係する連絡先リストの読み取り | サインインしたユーザー、またはサインインしているユーザーの組織内の他のユーザーに関連する、ユーザーのスコアの付いたリストを、アプリで読み取れるようにします。 <br/><br/>リストには、個人の連絡先、ソーシャル ネットワーキングまたは組織のディレクトリからの連絡先、最近 (電子メール、Skype などで) 連絡した人を含めることができます。 また、サインインしているユーザーの組織のディレクトリ全体を、アプリで検索することもできます。 | はい |
...
|   アクセス許可    |  表示文字列   |  説明 | 管理者の同意が必要​ |
|:----------------|:------------------|:-------------|:-----------------------|
| _SecurityEvents.Read.All_        |  組織のセキュリティ イベントの読み取り | アプリで、組織のセキュリティ イベントを読み取れるようにします。 | はい  |
...
|   アクセス許可    |  表示文字列   |  説明 | 管理者の同意が必要​ |
|:----------------|:------------------|:-------------|:-----------------------|
| _User.Read.All_ |    すべてのユーザーの完全なプロファイルの読み取り | サインインしているユーザーなしで、アプリで組織内の他のユーザーのプロファイル プロパティ、グループ メンバーシップ、部下、上司の完全なセットを読み取ることができるようにします。| はい |
...
| _User.Invite.All_  |     組織へのゲスト ユーザーの招待 | サインインしているユーザーなしで、ゲスト ユーザーをアプリで組織に招待できるようにします。 | はい |
| _User.Export.All_       |    ユーザーのデータのエクスポート | アプリで、サインインしているユーザーなしで、組織ユーザーのデータをエクスポートできるようにします。| はい |
...
|アクセス許可    |表示文字列   |説明 |管理者の同意が必要​ |
|:-----------------------------|:-----------------------------------------|:-----------------|:-----------------|
| _Notifications.ReadWrite.CreatedByApp_ | アプリ用の通知を送信し管理します。 | サインインしているユーザーに代わって、アプリが通知を送信出来るようにします。 アプリが、ユーザーのアプリ用通知アイテムの読み取り、更新、および削除が出来るようにもします。 |いいえ |
### <a name="remarks"></a>注釈
*UserActivity.ReadWrite.CreatedByApp* は、Microsoft アカウントと職場または学校アカウントのどちらでも有効です。 このアクセス許可に伴う *CreatedByApp* 制約は、呼び出しアプリのID、すなわちMicrosoft アカウントのアプリIDか、クロスプラットフォームアプリID用に設定されたアプリIDの一組かに準じて、このサービスが結果に対しフィルターを暗黙的に適用することを示しています
 
### <a name="example-usage"></a>使用例
#### <a name="delegated"></a>委任
* _Notifications.ReadWrite.CreatedByApp_: ユーザー中心型の通知を公開します。その後その通知は、別のエンドポイントで実行されているユーザーの複数のアプリ クライアントに配信する場合があります。 (投稿/自分/通知/)。
