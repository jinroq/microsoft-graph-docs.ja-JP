...
すべてのアクセス許可は、Microsoft アカウントと作業の両方に有効なまたは学校のアカウントです。 **Microsoft アカウントがサポートされている**列の特定のアクセス許可が Microsoft アカウント、作業や学校のアカウント、またはその両方に対して有効かどうかを確認するには、各アクセス許可グループをチェックすることができます。 
...
|:-----------------------------|:-----------------------------------------|:-----------------|:-----------------|
| _AppCatalog.ReadWrite.All_ | 読み取りおよび書き込みアプリケーションのすべてのカタログ  | 作成、読み取り、更新、およびアプリケーションのカタログでアプリケーションを削除するようにアプリケーションを使用できます。 | はい |
...
現在のみのカタログは、[マイクロソフトのチーム](teams-concept-overview.md)でのアプリケーションの一覧です。

### <a name="example-usage"></a>使用例

#### <a name="delegated"></a>委任
* _AppCatalog.ReadWrite.All_:[カタログ内のすべてのアプリケーションを一覧表示](/graph/api/teamsapp-list?view=graph-rest-beta)(`GET /beta/appCatalogs/teamsApps`)
* _AppCatalog.ReadWrite.All_:[アプリケーションを発行](/graph/api/teamsapp-publish?view=graph-rest-beta)(`POST /beta/appCatalogs/teamsApps`)
* _AppCatalog.ReadWrite.All_:[公開されているアプリケーションを更新](/graph/api/teamsapp-update?view=graph-rest-beta)(`PATCH /beta/appCatalogs/teamsApps/{id}`)
* _AppCatalog.ReadWrite.All_:[発行されたアプリケーションを削除する](/graph/api/teamsapp-delete?view=graph-rest-beta)(`DELETE /beta/appCatalogs/teamsApps/{id}`)
...
|   アクセス許可    |  表示文字列   |  説明 | 管理者の同意の要不要 | サポートされている Microsoft アカウント |
|:----------------|:------------------|:-------------|:-----------------------|:--------------|
...
## <a name="calls-permissions"></a>呼び出しのアクセス許可
...
|アクセス許可    |表示文字列   |説明 |管理者の同意の要不要 |
|:-----------------------------|:-----------------------------------------|:-----------------|:-----------------|
|_Calls.Initiate.All_|アプリケーション (プレビュー) から 1:1 の発信呼び出しを開始します。|サインイン中のユーザーの組織のディレクトリ内のユーザーに単一のユーザーと転送の呼び出しへの発信コールを配置するようにアプリケーションを使用できます。|はい|
|_Calls.InitiateGroupCall.All_|(プレビュー) アプリケーションからの呼び出しのグループを送信を開始|複数のユーザーへの発信コールを配置し、サインイン中のユーザー、組織内の会議に参加者を追加するようにアプリケーションを使用できます。|はい|
|_Calls.JoinGroupCall.All_|アプリ (プレビュー) とグループ通話や会議に参加します。|サインインしているユーザーに、組織のグループの呼び出しとスケジュールされたミーティングに参加するようにアプリケーションを使用できます。 アプリケーションは、テナント内の会議ディレクトリ ユーザーの特権を使用して結合されます。|はい|
|_Calls.JoinGroupCallasGuest.All_|ゲスト (プレビュー) としてグループ通話や会議に参加します。|匿名でログインしているユーザーに、組織のグループ呼び出しおよび予約されたミーティングに参加するようにアプリケーションを使用できます。 アプリケーションは、テナント内の会議にゲストとして参加できます。|はい|
|_Calls.AccessMedia.All_\*|アプリ (プレビュー) としての呼び出しでアクセスのメディア ストリーム|呼び出し、サインインしているユーザーがいない状態でメディア ストリームへの直接アクセスを取得するアプリケーションを使用できます。|はい|

> \***重要な:** 記録またはそれ以外の場合の呼び出しや、ボットにアクセスするための会議からのメディアの内容を保持する Microsoft.Graph.Calls.Media API を使用することがあります。
...
* _Calls.Initiate.All_: 組織内のユーザーにアプリケーションからピア ツー ピアの呼び出しを行う (`POST /beta/app/calls`)。
* _Calls.InitiateGroupCall.All_: 組織内のユーザーのグループをアプリケーションから呼び出すグループ (`POST /beta/app/calls`)。
* _Calls.JoinGroupCall.All_: グループまたはアプリケーションからオンライン会議に参加 (`POST /beta/app/calls`)。
* _Calls.JoinGroupCallasGuest.All_: グループに参加または会議でのみは、ゲストの特権のアプリケーション以外のアプリケーションからオンライン会議 (`POST /beta/app/calls`)。
* _Calls.AccessMedia.All_: 作成または呼び出しを結合し、アプリケーションの呼び出しでの参加者のメディア ストリームへの直接アクセスを取得する (`POST /beta/app/calls`)。

> **注:** 要求の例については、[呼び出しを作成する](/graph/api/application-post-calls?view=graph-rest-beta)を参照してください。
...
| _PrivilegedAccess.ReadWrite.AzureAD_ |ディレクトリの Id 管理の権限を持つデータの読み書き  | Azure AD の読み取りをアプリケーションと管理者の Id 管理 Api への書き込みアクセスをできます。 | はい | いいえ |
| _PrivilegedAccess.ReadWrite.AzureRBAC_ |Azure のリソースの Id 管理の権限を持つデータの読み書き | アプリケーションに読み取りと書き込みのアクセス権限を持つユーザーの管理 Api を Azure のリソースのことができます。 | はい | いいえ |
...
- アプリケーションのスキーマ拡張の定義
> **メモ**:
> - ユーザーのパスワードをリセットする権限がありません。
> - リソース (ユーザーまたはグループを含む) を削除する権限がありません。
> - 明示的に除外を作成またはリソースがない上に表示されているを更新します。 これが含まれています: アプリケーション、oAauth2Permissiongrant、appRoleAssignment、デバイス、servicePrincipal、組織、ドメイン、およびようにします。
...
> **注**: 個人のアカウントでは、Files.Read と Files.ReadWrite もへのアクセス許可、サインイン中のユーザーと共有されているファイルです。 
...
---
## <a name="financials-permissions"></a>金融サービスのアクセス許可
#### <a name="delegated-permissions"></a>委任されたアクセス許可

|アクセス許可|表示文字列|説明|管理者の同意の要不要|
|:----------|:--------------|:-----------|:-------|
|_Financials.ReadWrite.All_|財務データを読み書きします。|サインインしているユーザーのための財務データを読み書きするため、アプリケーション|いいえ|
...
|   アクセス許可    |  表示文字列   |  説明 | 管理者の同意の要不要 |
|:-----------------------------|:-----------------------------------------|:-----------------|:-----------------|
| _Group.Read.All_ | すべてのグループの読み取り | サインインしているユーザーがいない状態のすべてのグループのメンバーシップを読み取るアプリケーションを使用できます。 > **注:** アプリケーション専用のアクセス許可を使用してアクセスをサポートしている API をグループ化するわけではありません。 [既知の問題](known-issues.md)の例についてを参照してください。 | 必要 |
| _Group.ReadWrite.All_ | すべてのグループの読み取りと書き込み | グループを作成、読み取りとグループのメンバーシップを更新およびグループを削除するようにアプリケーションを使用できます。 ユーザの署名のないアプリケーションでは、これらの操作すべてを実行できます。 > **注:** アプリケーション専用のアクセス許可を使用してアクセスをサポートしている API をグループ化するわけではありません。 [既知の問題](known-issues.md)の例についてを参照してください。| はい |
...
また、グループのアクセス許可は、[Microsoft Planner](/graph/api/resources/planner-overview?view=graph-rest-beta) のリソースと API へのアクセスを制御するためにも使用されます。Microsoft Planner API では委任されたアクセス許可のみがサポートされます。アプリケーション アクセス許可はサポートされません。個人用 Microsoft アカウントはサポートされません。
...
* _Group.ReadWrite.All_: グループのメンバーを更新 (`POST /groups/{id}/members/$ref`)。 
> **注:**: _User.ReadBasic.All_をメンバーとして追加するユーザーを読み取ることも必要です。
...
## <a name="identity-risky-user-permissions"></a>Id 危険なユーザーのアクセス許可

#### <a name="delegated-permissions"></a>委任されたアクセス許可

|   アクセス許可    |  表示文字列   |  説明 | 管理者の同意の要不要 | サポートされている Microsoft アカウント |
|:----------------|:------------------|:-------------|:-----------------------|:--------------|
| _IdentityRiskyUser.Read.All_ |   Id ユーザーのリスク情報を読み取る  | サインインしているユーザーの代理として、組織内のすべてのユーザー id ユーザーのリスク情報を読み取るアプリケーションを使用できます。 | はい | いいえ |

#### <a name="application-permissions"></a>アプリケーションのアクセス許可

|   アクセス許可    |  表示文字列   |  説明 | 管理者の同意の要不要 |
|:-----------------------------|:-----------------------------------------|:-----------------|:-----------------|
| _IdentityRiskyUser.Read.All_ |   Id ユーザーのリスク情報を読み取る | サインイン中のユーザー、組織内のすべてのユーザー id ユーザーのリスク情報を読み取るアプリケーションを使用できます。 | はい |


### <a name="remarks"></a>解説

_IdentityRiskyUser.Read.All_は、職場または学校のアカウントに対してのみ有効です。 サインイン中のユーザー id のユーザーのリスク情報を読み取るための委任されたアクセス許可でアプリケーションの管理者の役割を次のいずれかのメンバーである必要があります: グローバル管理者、セキュリティ管理者、またはセキュリティのリーダーです。 管理者ロールの詳細については、「[Azure Active Directory での管理者ロールの割り当て](https://docs.microsoft.com/azure/active-directory/active-directory-assign-admin-roles)」を参照してください。
...
* すべてのリスクの高いユーザーとテナントのプロパティを読み取る (`GET /beta/riskyUsers`)
* 集計リスクのレベルが [中] ではすべてのリスクのあるユーザーの読み取り (`GET /beta/riskyUsers?$filter=risk/riskLevelAggregated eq microsoft.graph.riskLevel'medium'`)
* 特定のユーザーのリスク情報を読み取る (`GET /beta/riskyUsers/$filter=id eq ‘{userObjectId}’`)
...
| _Notes.ReadWrite.CreatedByApp_ |    ノートブックへの限定的なアクセス (非推奨) | **非推奨** <br/>使用しないでください。このアクセス許可によって付与される特権はありません。 | なし | いいえ
...
|アクセス許可    |表示文字列   |説明 |管理者の同意の要不要 |
|:-----------------------------|:-----------------------------------------|:-----------------|:-----------------|
| _Notifications.ReadWrite.CreatedByApp_ | 提供し、このアプリケーションの通知を管理します。 | ログインしているユーザーのための通知を配布するアプリケーションを許可します。 アプリケーションの読み取り、更新、および、このアプリケーションのユーザーの通知の項目を削除することもできます。 |いいえ |
### <a name="remarks"></a>解説
*Notifications.ReadWrite.CreatedByApp*は、Microsoft アカウントと作業の両方に有効なまたは学校のアカウントです。 このアクセス許可に関連付けられている*CreatedByApp*の制約またはことを示します、サービスが呼び出し元のアプリケーションでは、いずれかの Microsoft アカウント アプリケーション ID の id に基づく結果を暗黙にフィルターを適用するはのアプリケーション Id のセットが構成されている、クロスプラット フォームのアプリケーション id です。 
### <a name="example-usage"></a>使用例
#### <a name="delegated"></a>委任
* _Notifications.ReadWrite.CreatedByApp_: 別のエンドポイントで実行されている、ユーザーの複数のアプリケーション クライアントに配信できる可能性がありますユーザー中心型の通知を発行します。 (投稿/me/通知/)。
...
|アクセス許可    |表示文字列   |説明 |管理者の同意の要不要 |
|:-----------------------------|:-----------------------------------------|:-----------------|:-----------------|
|_OnlineMeetings.Read.All_|アプリケーション (プレビュー) からの読み取りのオンライン会議の詳細|サインイン中のユーザー、組織内のオンライン会議の詳細を参照するようにアプリケーションを使用できます。|はい|
|_OnlineMeetings.ReadWrite.All_|読み取りおよびユーザーのためのアプリケーション (プレビュー) からオンライン会議を作成|サインイン中のユーザー、ユーザーの代理として、組織内のオンライン会議を作成するアプリケーションを使用できます。|はい|
...
* _OnlineMeetings.Read.All_: プロパティと、[オンライン会議](/graph/api/onlinemeeting-get?view=graph-rest-beta)の関係を取得する (`GET /beta/app/onlinemeetings/{id}`)。
* _OnlineMeetings.ReadWrite.All_:[オンライン会議](/graph/api/application-post-onlinemeetings?view=graph-rest-beta)を作成 (`POST /beta/app/onlinemeetings`)。

> **注**: 要求の本文に指定されたユーザーの代わりに会議を作成、[オンライン会議](/graph/api/application-post-onlinemeetings?view=graph-rest-beta)を作成できますが、ユーザーの予定表では表示されません。
...
| _Sites.Manage.All_      | すべてのサイト コレクションにおけるアイテムとリストの作成、編集、および削除 | サインイン ユーザーなしで、すべてのサイト コレクションに含まれるリスト、ドキュメント、およびリスト アイテムをアプリで管理および作成できるようにします。  | はい  |
| _Sites.FullControl.All_ | すべてのサイト コレクションのフル コントロール | サインイン ユーザーなしで、アプリはすべてのサイト コレクション内の SharePoint サイトをフル コントロールできます。  | はい  |
...
|   アクセス許可    |  表示文字列   |  説明 | 管理者の同意の要不要 | サポートされている Microsoft アカウント |
|:----------------|:------------------|:-------------|:-----------------------|:--------------|
| _User.Read_       |    サインインとユーザー プロファイルの読み取り | ユーザーがアプリにサインインできるようにします。またサインインしているユーザーのプロファイルをアプリで読み取ることができるようにします。また、サインインしているユーザーの基本会社情報をアプリで読み取れるようにします。| なし | はい |
