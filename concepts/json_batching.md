...
個々の要求は、任意で、`headers` プロパティと `body` プロパティを含むこともできます。これら両方のプロパティは、前の例のように、通常 JSON オブジェクトです。一部の場合、`body` は JSON オブジェクトではなく、base64 で URL エンコードされた値となることがあります。たとえば、本文が画像である場合などです。これらの場合、`headers` オブジェクトには `content-type` の値を指定する必要があります。
...
* `method` と `url` ではなく、個々の応答には `status` プロパティが存在する。`status` の値は、HTTP ステータス コードを表します。
