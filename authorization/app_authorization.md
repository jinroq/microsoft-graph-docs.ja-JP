...
> **注:**上記の `302 Found` 応答は、`https://login.windows.net/{tenantId}/oauth2/authorize?...` URL に対してログイン処理を開始した場合に取得する `302 Found` 応答とは異なります。後者の場合、`302 Found` 応答は要求を `login.microsoftonline.com` に転送します。
