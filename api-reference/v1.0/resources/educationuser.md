...
|externalSource|`educationExternalSource`| このユーザーが作成された場所。 使用可能な値は`sis`、 `manual`、です。|
...
|primaryRole|educationUserRole| ユーザーの既定のロール。 ユーザーのロールは、個々のクラスで異なる場合があります。 使用可能な値は`student`、 `teacher`、です。 $filter をサポートします。|
...
|teacher|[educationTeacher](educationteacher.md)| プライマリロールが教師の場合、このブロックには教師固有のデータが含まれます。|
