...
|partnerState|文字列型 (String)|このテナントのパートナーの状態です。可能な値: `unknown`、`unavailable`、`enabled`、`terminated`、`rejected`、`unresponsive`。|
|partnerAppType|文字列型 (String)|パートナー アプリの種類です。可能な値: `unknown`、`singleTenantApp`、`multiTenantApp`。|
|singleTenantAppId|文字列型 (String)|パートナーのシングル テナントのアプリ ID|
|displayName|文字列型 (String)|パートナー表示名|
|isConfigured|ブール型 (Boolean)|デバイス管理パートナーが構成されているかどうか|
