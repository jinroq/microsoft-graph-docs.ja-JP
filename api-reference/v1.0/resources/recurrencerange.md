...
|recurrenceTimeZone|文字列 |**startDate** プロパティと **endDate** プロパティのタイム ゾーン。 省略可能。 指定されていない場合は、イベントのタイム ゾーンが使用されます。|
|startDate|日付|定期的なパターンの適用を開始する日付。 イベントの定期的なパターンによっては、最初に会議が発生するのは、この日付以降になる場合があります。 定期的な[イベント](event.md)の **start** プロパティと同じ値である必要があります。 必須。|
|型|RecurrenceRangeType|繰り返す期間。 指定できる値は、 `endDate`、`noEnd`、`numbered` です。 必須。|
