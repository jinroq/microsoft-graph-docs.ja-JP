...
|プロパティ|型|説明|
|:---|:---|:---|
|id|String|リソース操作のキーです。 読み取り専用で、自動生成されます。|
|resourceName|String|この操作が実行されるリソースの名前です。|
|actionName|String|この操作が実行するアクションの種類です。 actionName は簡潔で、できるだけ少ない単語にする必要があります。|
|description|String|リソース操作の説明です。 Azure Portal で操作にカーソルを合わせると、その操作の説明がテキストで表示されます。|
