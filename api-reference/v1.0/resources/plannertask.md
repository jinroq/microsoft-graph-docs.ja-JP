...
| メソッド           | 戻り値の型    |説明|
|:---------------|:--------|:----------|
|[PlannerTask を取得します](../api/plannertask_get.md) | [plannerTask](plannertask.md) |**plannerTask** オブジェクトのプロパティとリレーションシップを読み取ります。|
|[更新](../api/plannertask_update.md) | [plannerTask](plannertask.md) |**plannerTask** オブジェクトを更新します。 |
|[削除](../api/plannertask_delete.md) | なし |**plannerTask** オブジェクトを削除します。 |
...
|assigneePriority|文字列|リスト ビューでこの種類の項目の順序付けに使用するヒント。形式は[ここ](planner_order_hint_format.md)の説明に従って定義されます。|
|課題|[plannerAssignments](plannerassignments.md)|タスクが割り当てられている一連の担当者。|
|bucketId|文字列|タスクが属するバケット ID。 バケットは、タスクが含まれるプランになければなりません。 28 文字で大文字小文字を区別します。 サービスの[フォーマットの検証](planner_identifiers_disclaimer.md) が行われます。 |
...
|hasDescription|ブール値|読み取り専用。タスクの details オブジェクトが保持する説明が空でない場合、値は `true` になり、そうでない場合は `false` になります。|
|ID|文字列|読み取り専用。 タスクの ID。 28 文字で大文字小文字を区別します。 サービスの[フォーマットの検証](planner_identifiers_disclaimer.md) が行われます。|
...
|previewType|plannerPreviewType|タスクに表示されるプレビューの種類を設定します。 指定できる値は、`automatic`、`noPreview`、`checklist`、`description`、`reference`です。|
