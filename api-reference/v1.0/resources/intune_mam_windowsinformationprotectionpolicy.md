...
|pinMinimumLength|Int32|PIN に必要な文字の最小数を設定する整数値です。 既定値は 4 です。 このポリシー設定で構成できる最小値は 4 です。 構成できる最大値は、[PIN の最大文字数] ポリシー設定で構成された値、または 127 のうち、どちらか小さいほうです。|
...
|pinSpecialCharacters|String|Windows Hello for Business の PIN における特殊文字の使用を構成する整数値です。 !  " # $ % & ' ( ) * + , - . / : ; < = > ? @ \[ \ \] ^ _ ` { | } ~ が、Windows Hello for Business の PIN ジェスチャの有効な特殊文字です。 既定値は NotAllow です。 可能な値は、`notAllow`、`requireAtLeastOne`、`allow` です。|
