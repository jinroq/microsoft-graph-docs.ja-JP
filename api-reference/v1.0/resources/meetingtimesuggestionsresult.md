...
| プロパティ     | タイプ   |説明|
|:---------------|:--------|:----------|
|emptySuggestionsReason|文字列|会議提案が 1 つも返されない理由。 使用可能な値は、`attendeesUnavailable`、`attendeesUnavailableOrUnknown`、`locationsUnavailable`、`organizerUnavailable`、`unknown` です。  **MeetingTimeSuggestions** プロパティは、会議の提案を含める場合、このプロパティは空の文字列をします。|
