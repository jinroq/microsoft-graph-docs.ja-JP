# <a name="remoteitem-resource-type"></a>RemoteItem リソースタイプ

**RemoteItem** リソースは、[DriveItem](driveitem.md) が別のドライブに存在する項目を参照することを示します。このリソースは、ソース ドライブとターゲット項目の固有 ID を提供します。
...
**注:**同じドライブにあるフォルダーとは異なり、リモート アイテムに移動された DriveItem には、`id` 値が変更された可能性があります。
...
| parentReference      | [ItemReference](itemreference.md)             | リモート項目の親のプロパティです。読み取り専用。                  |
