# <a name="create-iosmanagedappprotection"></a>iosManagedAppProtection を作成します
...
|allowedOutboundDataTransferDestinations|[managedAppDataTransferLevel](../resources/intune_mam_managedappdatatransferlevel.md)|データの転送が許可された宛先。[managedAppProtection](../resources/intune_mam_managedappprotection.md) から継承されます。使用可能な値は、`allApps`、`managedApps`、`none` です。|
...
|appDataEncryptionType|[managedAppDataEncryptionType](../resources/intune_mam_managedappdataencryptiontype.md)|マネージ アプリケーションのデータに使用する暗号化の種類: 使用可能な値は`useDeviceSettings`、 `afterDeviceRestart`、 `whenDeviceLockedExceptOpenFiles`、 `whenDeviceLocked`。|
