...
|publishingState|String|アプリの発行状態。 アプリが発行されていないと、アプリを割り当てることができません。 [mobileApp](../resources/intune_apps_mobileapp.md) から継承します。可能な値は、`notPublished`、`processing`、`published` です。|
|packageId|String|パッケージの識別子。|
|appStoreUrl|String|Android アプリ ストアの URL。|
|minimumSupportedOperatingSystem|[androidMinimumOperatingSystem](../resources/intune_apps_androidminimumoperatingsystem.md)|適用可能なオペレーティング システムの最小の値です。|
