...
| feedback          | alertFeedback                                                          | アナリストのフィードバック通知をします。 使用可能な値は、`unknown`、`truePositive`、`falsePositive`、`benignPositive` です。 |
| status            | alertStatus                                                            | アラートのライフ サイクルのステータス (ステージ)。 使用可能な値は、`unknown`、`newAlert`、`inProgress`、`resolved` です。 |
