...
|comments|文字列コレクション|警告に対する顧客提供のコメント (顧客警告管理の場合) ([更新](../api/alert-update.md)をサポートしています)。|
...
|説明|String|警告の説明。|
|detectionIds|文字列コレクション|この警告エンティティに関連する警告のセット (各警告は個別のレコードとして SIEM にプッシュされます)。|
...
|recommendedActions|文字列コレクション|警告の結果として実行する仕入先/プロバイダー推奨アクション (たとえば、マシンの分離、enforce2FA、再イメージ化ホスト)。|
