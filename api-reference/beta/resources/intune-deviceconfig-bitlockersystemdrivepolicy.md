...
|startupAuthenticationRequired|ブール値|起動時に追加の認証が必要です。|
|startupAuthenticationBlockWithoutTpmChip|ブール値|なし (または USB フラッシュ ドライブにスタートアップ キーのパスワードが必要です)、互換性のある TPM は BitLocker を許可するかどうかを示します。|
...
|prebootRecoveryEnableMessageAndUrl|ブール値|ブート前の回復のメッセージと Url を有効にします。 RequireStartupAuthentication が false の場合、この値は影響しません。|
