...
|[アイテムの投稿](../api/dynamics-create-item.md)  |items     |アイテムオブジェクトを作成します。|
...
|type                |numeric |アイテムの在庫の種類。 1 = 在庫アイテム、2 = サービスアイテム。 これは必須プロパティです。|
...
|gtin                |numeric |これは、グローバルな取引項目番号です。                |
...
|taxGroupCode        |numeric |税グループは、同一の税用語の対象となる在庫品目またはリソースのグループを表します。|
|lastModifiedDateTime|datetime|アイテムが最後に変更された datetime。 読み取り専用です。  |  
