...
| hasError | ブール型 | ルールがエラー状態かどうかを示します。 読み取り専用です。 |
| id |String|ルールの一意識別子。 読み取り専用。|
| isEnabled | ブール型 | メッセージに対するルールの適用が有効になっているかどうかを示します。 |
...
|[削除する](../api/messagerule-delete.md) | なし |指定した **messageRule** オブジェクトを削除します。 |
