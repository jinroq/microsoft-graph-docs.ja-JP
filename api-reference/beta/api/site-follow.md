...
---
# <a name="follow-site"></a>サイトをフォローする 

ユーザーの[サイト](../resources/site.md)または複数のサイトに従います。
...
* 要求が成功した場合、このメソッドは、フォローされたサイトの配列を返します。  
* 指定したサイトのフォロー中にエラーが発生した場合、この`207`メソッドは状態コードを返し、応答本文には、フォローできないサイトを示す[Error](/graph/errors)オブジェクトと siteids を含むエントリの配列が含まれます。
