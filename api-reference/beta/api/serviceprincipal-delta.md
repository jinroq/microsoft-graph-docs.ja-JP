...
Get を新しく作成するには、更新、または全体のリソースのコレクションのすべての読み取りを実行することがなくサービス ・ プリンシパルを削除します。 詳細については、[デルタのクエリを使用する](/graph/delta-query-overview)を参照してください。
...
変更の追跡を開始するには、servicePrincipal リソースでデルタ関数を含む要求を行います。 
...
かどうかは成功すると、このメソッドを返します`200 OK`、応答の本体で応答コードと[servicePrincipal](../resources/serviceprincipal.md)コレクションのオブジェクトです。 応答には、nextLink URL または deltaLink の URL も含まれています。 
