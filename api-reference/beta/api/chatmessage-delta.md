...
2 番目の要求では、前の応答で返された `nextLink` URL が指定されます。 最初の要求と同じ top パラメーターを指定する必要がない点に注意してください。これは、`nextLink` URL の `skipToken` によってエンコードされて含まれるためです。
...
3 番目の要求では、最後の同期要求から返された最新の `nextLink` を引き続き使用します。
