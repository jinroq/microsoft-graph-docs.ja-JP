...
要求本文で、win32LobApp オブジェクトの JSON 表記を指定します。

次の表に、win32LobApp の作成時に必要なプロパティを示します。
...
|installcommandline|文字列|このアプリをインストールするためのコマンドライン|
|アン installcommandline|文字列|このアプリをアンインストールするためのコマンドライン|
...
|setupfilepath|文字列|暗号化された Win32LobApp パッケージ内のセットアップファイルの相対パス。|



## <a name="response"></a>応答
成功した場合、このメソッド`201 Created`は応答コードと、応答本文で[win32LobApp](../resources/intune-apps-win32lobapp.md)オブジェクトを返します。
